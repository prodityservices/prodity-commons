...
-- this view only contains the id, name, and
...
CREATE VIEW `bungee_name_last_seen` (player_name, player_last_seen) AS
  SELECT player_name, MAX(player_last_seen) AS player_last_seen FROM bungee_name_cache GROUP BY player_name;
CREATE VIEW `bungee_names`(player_id, player_name, player_last_seen) AS
  SELECT player_id, player_name, player_last_seen FROM bungee_name_cache
  INNER JOIN `bungee_name_last_seen`
  USING (player_name, player_last_seen);