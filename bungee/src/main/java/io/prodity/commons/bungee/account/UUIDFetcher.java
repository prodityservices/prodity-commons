...
public class UUIDFetcher implements Callable<Map<String, UUID>> {
	...
	public static Optional<UUID> getUUIDOf(String name) throws Exception {
		return Optional.ofNullable(new UUIDFetcher(Collections.singletonList(name)).call().get(name));
	}

	@Override
	public Map<String, UUID> call() throws Exception {
		Map<String, UUID> uuidMap = new TreeMap<>();
		...
		for (int i = 0; i < requests; i++) {
			...
			JSONArray array = (JSONArray) UUIDFetcher.PARSER.get().parse(new InputStreamReader(connection.getInputStream()));
